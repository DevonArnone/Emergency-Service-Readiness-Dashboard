...
export default function CredentialsPage() {
  ...
  return (
    <div className="ops-page">
      <div className="ops-shell space-y-6">
        ...
        <div className="surface-header">
          <div>
            ...
            <h1 className="mt-1.5 text-2xl font-semibold tracking-tight text-white md:text-4xl">Credentials</h1>
        ...
        <div className="ops-panel">
          ...
          <div className="mt-3 mb-4 flex flex-wrap gap-2 items-center">
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search…" className="form-control py-1.5" />
            {(['ALL', ...categories] as string[]).map((c) => (
              <button key={c} onClick={() => setCatFilter(c)} className={`filter-chip ${catFilter === c ? 'filter-chip-active' : ''}`}>
                {c === 'ALL' ? 'All' : c}
              </button>
            ))}
          </div>
      ...
      {/* Add / edit modal */}
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-xl" onClick={() => { setShowAdd(false); setEditTarget(null) }}>
          <div className="w-full max-w-md rounded-[26px] border border-white/10 bg-slate-950/90 p-6 shadow-[0_34px_120px_rgba(0,0,0,0.58)]" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={saveCert} className="space-y-4">
              <div>
                ...
                <input required name="name" defaultValue={editTarget?.name} className="form-control w-full" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Category</label>
                <input name="category" defaultValue={editTarget?.category} placeholder="Fire, EMS, Rescue…" className="form-control w-full" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Description</label>
                <input name="description" defaultValue={editTarget?.description} className="form-control w-full" />
              </div>
              <div>
                <label className="block text-xs text-slate-400 mb-1">Typical validity (days)</label>
                <input type="number" name="validity" defaultValue={editTarget?.typical_validity_days} min={1} placeholder="365" className="form-control w-full" />
              </div>
